...
import com.epicspymain.isrealanything.events.helpers.ChunkDestroyer;
import com.epicspymain.isrealanything.events.helpers.TNTSpawner;
 ...
 * - Killing TheME/TheOtherME entities before Event 41
...
public class TheOverlook {
    ...
    private static void beginMeltdown(ServerPlayerEntity player, MinecraftServer server) {
        ...
        // Force all remaining events except Event 42
        EventManager.forceRunAllExcept(42);
    ...
    private static void spawnWarningSign(ServerWorld world, BlockPos pos, String text) {
        ...
        if (world.getBlockEntity(pos) instanceof SignBlockEntity sign) {
            ...
            for (int i = 0; i < Math.min(lines.length, 4); i++) {
                signText = signText.withMessage(i, 
    ...
    /**
     * begone T H O T crash
     */
    private static void crash(ServerPlayerEntity player) {
        // Display messages first
        player.sendMessage(Text.literal("MINECRAFT HAS CRASHED").formatted(Formatting.RED, Formatting.BOLD), false);
        player.sendMessage(Text.literal("Don't Say I Didn't Warn You").formatted(Formatting.DARK_RED), false);

        // ACTUALLY CRASH
        throw new RuntimeException("TheOverlook.ForbiddenAction - Don't Say I Didn't Warn You");
    }
    ...
    public static void onEntityDeath(ServerPlayerEntity killer, LivingEntity victim) {
        ...
        // Check if Event 41 has occurred
        if (EventManager.hasEventOccurred(41)) {
            return; // Event 41 occurred, death is allowed
        }
    ...
    public static void checkEntityRemoval(Entity entity, World world) {
        ...
        if (entity.isRemoved() && !entity.isDead() && !EventManager.hasEventOccurred(38)) {
